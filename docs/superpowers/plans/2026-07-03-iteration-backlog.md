...
- [x] **范围（建议分两步）：**
  ...
  - ~~**5c（可选）** 忘记密码 / 邮箱重置~~ → **明确不做**（2026-07-03 用户决定）
...
- [x] **范围（第一阶段）：**
  ...
  - Service Worker / 离线缓存（vite-plugin-pwa + workbox 预缓存静态资源）
- [x] **验证：** 移动端浏览器可「添加到主屏幕」；manifest 可访问；生产构建生成 `sw.js`

### 12. 图片云存储（OSS）— 明确不做

- [x] **状态：** **不实施**（2026-07-03 用户决定；继续本地 `uploads/` + `scripts/backup.sh`）
- ~~抽象存储层、OSS 迁移、签名 URL~~

### 13. PDF 直接下载（非打印）

- [x] **目标：** 一键下载 PDF 文件，无需打印对话框
- [x] **范围：**
  - 方案：嵌入中文字体到 jspdf，或后端 pdfkit 生成
  - 替换或补充现有 `exportRecordsToPdf` 打印方案
- [x] **验证：** 下载的 PDF 中文正常、表格完整

### 14. 导出增强

- [x] **目标：** 导出含附件信息或按筛选条件命名
- [x] **范围：** Excel 增加附件数 + 记录详情链接列；PDF 表格增加附件数列；文件名含筛选关键词
- [x] **验证：** 导出文件与当前筛选条件一致；Excel 含可跳转的记录链接
- ~~PDF 缩略图~~ → 工作量大，暂不实施
...
- [x] **状态：** **不实施**（2026-07-03 用户决定；手动测试 + 集成测试已覆盖核心流程）
- ~~Playwright / Cypress happy path~~
...
### 19. API 文档（Swagger）— 明确不做

- [x] **状态：** **不实施**（2026-07-03 用户决定；参考 `docs/design.md` 与 `CLAUDE.md` API 速查即可）
- ~~swagger-jsdoc + swagger-ui-express~~

### 20. 错误日志与监控 — 明确不做

- [x] **状态：** **不实施**（2026-07-03 用户决定；个人项目依赖 PM2 / 控制台日志即可）
- ~~winston/pino、Sentry~~
...
- [x] **目标：** CSS 变量 + 切换开关；主要页面可读
- [x] **范围：** `useTheme` + `ThemeToggle` + `theme.css` dark 变量；时间轴等组件改用 CSS 变量
- [x] **验证：** 切换后首页/时间轴/趋势/详情可读

### 25. 多语言（i18n）— 明确不做

- [x] **状态：** **不实施**（2026-07-03 用户决定；界面保持中文）
- ~~vue-i18n~~

### 26. 移动端下拉刷新

- [x] **目标：** 首页与时间轴支持下拉刷新列表
- [x] **范围：** `usePullToRefresh` composable；修正 window 滚动检测
- [x] **验证：** 页面顶部下拉可刷新数据
...
| PWA | 项目称 Web 应用（PWA） | manifest + 可添加主屏幕 + SW 预缓存 | 任务 11 已完成 |
